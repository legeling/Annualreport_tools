...
class CNINFOClient:
    ...
    @staticmethod
    def _backoff_sleep(attempt: int, base: float = 1.0, cap: float = 10.0) -> None:
        """指数退避 + 抖动，降低短时高频重试带来的风控风险。"""
        ...
        for attempt in range(1, self.config.max_retries + 1):
            try:
                ...
                if response.status_code == 429:
                    logging.warning(f"触发限流429 (尝试 {attempt}/{self.config.max_retries}): {date_range} 第{page_num}页")
            ...
            if attempt < self.config.max_retries:
                self._backoff_sleep(attempt, base=float(self.config.retry_delay), cap=20.0)
